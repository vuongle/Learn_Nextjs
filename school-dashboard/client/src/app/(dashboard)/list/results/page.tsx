...
const ResultListPage = async ({
  ...
  const { userId, role } = await clerkAuth();
  ...
  // convert fields in db to fields in ResultList type
  const data = rawData.map((item) => {
    // specify the item is the result of an assignment or an exam
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      {/* PAGINATION */}
      <Pagination page={_page} count={count} />
    </div>
  );
};
